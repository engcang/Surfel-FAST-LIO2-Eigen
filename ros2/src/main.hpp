...
private:
    ...
    void pcdCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &_msg)
    {
        ...
        {
            RCLCPP_ERROR(rclcpp::get_logger("surfel_fast_lio2_eigen"), "lidar loop back, clear buffer");
            lidar_buffer_.clear();
        }
    ...
    void imuCallback(const sensor_msgs::msg::Imu::ConstSharedPtr &_msg_in)
    {
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("surfel_fast_lio2_eigen"), "imu loop back, clear buffer");
            imu_buffer_.clear();
        }
    ...
    void buildMeasurementModelJacobianMatrix(LioState &_s, DynamicSharedData &_measurement_data)
    {
        ...
        if (num_effective_points_ < 1)
        {
            ...
            RCLCPP_WARN(rclcpp::get_logger("surfel_fast_lio2_eigen"), "No Effective Points!");
            return;
        }
...
public:
    ...
    int mainFunction()
    {
        ...
        voxel_resolution_ = node->declare_parameter<double>("preprocess.voxel_resolution", 0.5);
        local_map_box_size_ = node->declare_parameter<double>("mapping.local_map_box_size", 1000.0);
        ...
        while (status)
        {
            ...
            if (synchronizeMeasurements(measurements_))
            {
                ...
                {
                    RCLCPP_WARN(rclcpp::get_logger("surfel_fast_lio2_eigen"), "No point, skip this scan!");
                    continue;
                }
