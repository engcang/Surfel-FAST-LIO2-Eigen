...
public:
    ...
    SurfelVoxelMap();
    explicit SurfelVoxelMap(const Parameters &_parameters);
    ...
    [[nodiscard]] bool findSurfel(const Eigen::Vector3d &_point, Surfel &_surfel) const;
...
private:
    ...
    using LeafMap = ankerl::unordered_dense::map<VoxelKey, LeafVoxel, VoxelKeyHash>;
    ...
    void insertPoint(const Eigen::Vector3f &_point);
...
inline SurfelVoxelMap::SurfelVoxelMap():
    SurfelVoxelMap(Parameters{})
{
}

inline SurfelVoxelMap::SurfelVoxelMap(const Parameters &_parameters)
{
    configure(_parameters);
}

inline void SurfelVoxelMap::configure(const Parameters &_parameters)
{
    if (!(_parameters.leaf_voxel_size_ > 0.0F))
    {
        throw std::invalid_argument("Surfel leaf size must be positive");
    }
    if (!(_parameters.map_half_extent_ > 0.0F))
    {
        throw std::invalid_argument("Surfel map half extent must be positive");
    }
    if (_parameters.minimum_occupied_leaf_count_ < 3U || _parameters.minimum_occupied_leaf_count_ > 27U)
    {
        throw std::invalid_argument("Surfel minimum occupied leaf count must be in [3, 27]");
    }
...
inline void SurfelVoxelMap::clear()
{
    leaves_.clear();
    parents_.clear();
...
inline void SurfelVoxelMap::update(const pcl::PointCloud<pcl::PointXYZINormal> &_points,
                                   const Eigen::Vector3d &_sensor_position)
{
    if (_points.empty())
    {
    ...
    leaves_.reserve(leaves_.size() + _points.size());
    parents_.reserve(parents_.size() + _points.size() / 8U + 1U);
    dirty_parents_.reserve(_points.size() / 4U + 1U);
    for (const pcl::PointXYZINormal &point : _points)
    {
        insertPoint(Eigen::Vector3f(point.x, point.y, point.z));
    }
    recomputeDirtySurfels();
}

inline bool SurfelVoxelMap::findSurfel(const Eigen::Vector3d &_point, Surfel &_surfel) const
{
...
inline bool SurfelVoxelMap::empty() const noexcept
{
    return leaves_.empty();
}

inline std::size_t SurfelVoxelMap::voxelCount() const noexcept
{
    return leaves_.size();
}

inline std::size_t SurfelVoxelMap::surfelCount() const noexcept
{
...
inline std::int32_t SurfelVoxelMap::floorDivideByThree(const std::int32_t _value) noexcept
{
    return _value >= 0 ? _value / 3 : (_value - 2) / 3;
}

inline void SurfelVoxelMap::insertPoint(const Eigen::Vector3f &_point)
{
    const VoxelKey leaf_key = pointToLeafKey(_point);
    auto [leaf_iterator, inserted] = leaves_.try_emplace(leaf_key);
    LeafVoxel &leaf = leaf_iterator->second;
    if (inserted)
    {
...
inline void SurfelVoxelMap::recomputeDirtySurfels()
{
    dirty_nodes_.clear();
...
inline void SurfelVoxelMap::recomputeSurfel(const VoxelKey &_parent_key, ParentVoxel &_parent)
{
    ...
    for (std::int32_t x_offset = 0; x_offset < 3; ++x_offset)
    {
        for (std::int32_t y_offset = 0; y_offset < 3; ++y_offset)
        {
            for (std::int32_t z_offset = 0; z_offset < 3; ++z_offset)
            {
                ...
                {
                    centroids[centroid_count++] = leaf_iterator->second.centroid_;
                }
            }
        }
    }
...
inline void SurfelVoxelMap::rebuildParents()
{
    parents_.clear();
    dirty_parents_.clear();
    surfel_count_ = 0U;
...
inline void SurfelVoxelMap::pruneIfNeeded(const Eigen::Vector3f &_sensor_position)
{
